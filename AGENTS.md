...
- **Center the meaningful content:** evaluate the visible label or number itself, not just its wrapper. Keep values centered in both axes before and after submission. Do not add repeated captions or reserve empty caption space when the equation and feedback already explain the reviewed values.
...
- **One clear statement of a result:** name a correction explicitly as the correct answer, and keep it next to the submitted equation. Avoid repeating “Chosen” or “Correct” beneath every reviewed option when that information is already conveyed by the submission and correction. Keep the answer tiles visually numeric, with programmatic descriptions of the selected and correct options for assistive technology. Removing repeated captions must preserve meaningful written feedback and must not make the result depend on color alone.
