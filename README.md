...
With **Choose answer**, tap one of four shuffled answers to check it immediately. Wrong options come from nearby factor mistakes, with nearby positive values as fallbacks. The options stay in place after checking, with the submitted value in the equation, written feedback, and screen-reader descriptions for the reviewed options; choose **Next** when ready. There is no software keyboard or separate Check answer step. Both formats use the same scoring and written feedback, and Practice again retains the format.
